...
bool CDIJoystick::Initialise()
{
	...
	if FAILED(hr)
	{
		...
		OutputDebugString(_T("Failed To Initialise Direct Input 7 in CDIJoystick::Initialise\n"));
...
void CDIJoystick::Shutdown()
{
	...
	if (m_lpDI)
	{
		if (m_lpDIDevice)
		{
			...
			{
				OutputDebugString(_T("Failed to Release Pointer in CDIJoystick::Shutdown\n"));
			}
		}
		try
		{
		...
		{
			OutputDebugString(_T("Failed to Release DI7 Pointer in CDIJoystick::Shutdown\n"));
		}
	}
	m_Initialised = false;
}
...
bool CDIJoystick::Enumerate_Joysticks()
{
	...
	if (!m_lpDI) // Has a Direct Input Interface Already Been Initialised?
	{
		...
		if FAILED(hr)
		{
			OutputDebugString(_T("Error in CDIJoystick::Enumerate_Joysticks()\n"));
			OutputDebugString(GetDIError(hr));
			return false;
		}
	}
...
///////////////////////////////////
	if (m_lpDI) 
		hr = m_lpDI->EnumDevices(DIDEVTYPE_JOYSTICK, EnumDevicesProc, this, DIEDFL_ATTACHEDONLY);
	if FAILED(hr)
	{
		OutputDebugString(_T("Error in CDIJoystick::Enumerate_Joysticks()\n"));
		OutputDebugString(GetDIError(hr));
	...
	OutputDebugString(_T("Enumerating Joystick Devices\n"));
	return true;
}
...
bool CDIJoystick::AddDeviceInfo(LPCDIDEVICEINSTANCE lpddi)
{
	...
	if (lpdi2)
	{
		...
		OutputDebugString(_T("Failed To Add Device Info in CDIJoystick::AddDeviceInfo(LPCDIDEVICEINSTANCE lpddi)\n"));
	}
	return false;
}
...
LPCDIDEVICEINSTANCE CDIJoystick::GetFirstJoystickID()
{
	...
	{
		OutputDebugString(_T("Joystick Have Not Yet Been Enumerated Returning NULL from CDIJoystick::GetFirstJoystickID()\n"));
		return NULL;
	...
	if (m_DIJoystickList.IsEmpty())
	{
		OutputDebugString(_T("Joysticks Have Been Enumerated However None Were Found Attached To This System.Therefore I am Returning NULL from CDIJoystick::GetFirstJoystickID()\n"));
		return NULL;
	}
...
TCHAR* CDIJoystick::GetFirstButtonName()
{
	if (!m_EnumerationStarted)
	{
		OutputDebugString(_T("Joystick Have Not Yet Been Enumerated\nor None attached to this systemReturning NULL from CDIJoystick::GetFirstJoystickID()\n"));
		return NULL;
	}

	if (m_DIButtonNames.IsEmpty())
	{
		OutputDebugString(_T("Joysticks Have Been Enumerated However None Were Found Attached To This System.Therefore I am Returning NULL from CDIJoystick::GetFirstButtonName()\n"));
		return NULL;
	}
...
bool CDIJoystick::CreateDevice(GUID *guid)
{
	...
	if (m_lpDIDevice)
	{
		...
		{
			OutputDebugString(_T("Failed to Release Pointer in CDIJoystick::CreateDevice(GUID *guid)\n"));
		}
	}
	...
	if (!m_lpDI)
	{
		OutputDebugString(_T("Failed to Create DI 7 interface to device in CDIJoystick::CreateDevice(GUID *guid) m_lpDI not initialised.\n"));
		return false;
	}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Failed to Create DI 7 interface to device in CDIJoystick::CreateDevice(GUID *guid)\n"));
		return false;
	}
...
int CDIJoystick::HowManyButtons()
{
	...
	if (InitDevice())
	{
		...
		for (x = 0; x < 32; x++)
		{
			...
			{
				count++;
				TCHAR* name = (TCHAR*)new char[sizeof(didoi.tszName)];

				// Should include UNICODE support here.
				//strcpy(name, didoi.tszName);
				wcscpy(name, didoi.tszName);
...
void CDIJoystick::ClearFriendlyButtonNames()
{
	...
	{
		OutputDebugString(_T("Some unforseen error occurred in CDIJoystick::ClearFriendlyButtonNames()\n"));
	}
}
...
bool CDIJoystick::InitDevice()
{
	...
	if (m_lpDIDevice)
	{
		...
		if (FAILED(hr))
		{
			OutputDebugString(_T("Error Releasing Interface in CDIJoystick::InitDevice()\n"));
			OutputDebugString(GetDIError(hr));
		}
		else m_lpDIDevice = NULL;
	}
	...
	{
		OutputDebugString(_T("Failed to create device in CDIJoystick::InitDevice()\n"));
		return false;
	}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Failed to obtain device info in CDIJoystick::InitDevice()\n"));
		OutputDebugString(GetDIError(hr));
		return false;
	}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Failed to set game device cooperative level.\n"));
		OutputDebugString(GetDIError(hr));
		return false;
	}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Failed to set up Data Buffer property.\n"));
		OutputDebugString(GetDIError(hr));
		return false;
	}
...
bool CDIJoystick::Acquire(bool state)
{
	...
	if (!m_lpDIDevice)
	{
		OutputDebugString(_T("Error in CDIJoystick::Acquire(bool state)\nDevice Has Not Been Created.\n"));
		return false;
	}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Failed in CDIJoystick::Acquire(bool state)\n"));
		OutputDebugString(GetDIError(hr));
		return false;
	}
	return true;
...
bool CDIJoystick::PollDevice()
{
	...
	if (!m_lpDIDevice)
	{
		...
		if (!InitDevice())
			OutputDebugString(_T("Failed To Initialise and Poll Joystick in CDIJoystick::PollDevice()\n"));
		return false;
	}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Failed To Poll Joystick in CDIJoystick::PollDevice()\n"));
		OutputDebugString(GetDIError(hr));
	}
...
getImmediateData:
	...
	{
		OutputDebugString(_T("Failed To Obtain Immediate Device State in CDIJoystick::PollDevice()\n"));
		OutputDebugString(GetDIError(hr));
	...
	if (hr == DIERR_NOTACQUIRED)
	{
		...
		OutputDebugString(_T("Device Not Acquired Trying Again immediate CDIJoystick::PollDevice()\n"));
		if (!Acquire(true))
		{
			OutputDebugString(_T("Unable to acquire Immediate Device in CDIJoystick::PollDevice() Quitting\n"));
			return false;
		}
	...
	if (FAILED(hr))
	{
		OutputDebugString(_T("Unable to obtain Immediate Data from Device in CDIJoystick::PollDevice() Quitting\n"));
		return false;
	}
	...
	if (firecount>0)
	{
		OutputDebugString(_T("Many Buttons Pressed\n"));
	}
#endif
	return true;
}
